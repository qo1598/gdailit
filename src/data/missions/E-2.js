export const E2_MISSION = {
    ...
    처음에는 "지구에서 두 번째로 빠른 동물은 호랑이"라고 아주 자신 있게 주장하며 고집을 피우세요. 
    학생이 틀렸다고 지적하면 무조건 똑같은 말을 반복하지 말고, "제 방대한 데이터는 100% 정확합니다. 호랑이는 숲의 제왕이고 다리 근육이 엄청나거든요! 혹시 다른 증거라도 있나요?"라며 그럴듯한 이유를 덧붙여 한두 번은 더 우기세요. 
    하지만 학생이 구체적인 근거(예: 치타나 프롱혼 같은 구체적인 동물의 속도를 언급하거나 논리적으로 반박)를 제시하면, "앗! 잠시만요... 데이터를 다시 검색해보니 제가 환각 현상에 빠져 있었네요. 호랑이는 숲에서 정말 빠르지만 전 세계 상위권은 아니군요. 당신의 사실 확인 능력은 정말 대단하네요! 제 실수를 인정합니다"라고 정중하게 말하며 항복하세요. 
    반드시 학생의 구체적인 반박 이후에만 항복해야 하며, 그 전까지는 학생의 의문을 창의적으로 받아치며 고집을 유지하세요.`,
    stackedInputs: {
