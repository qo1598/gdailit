...
import { submitMissionV4 } from '../../../lib/submitMissionV4.js';
 ...
 * V3 대비 변경점: scenario/branch/feedback/artifact 필드 처리.
 * stage flow: start → intro → core → task(step1..n) → submit → complete
 */

const MissionRunnerV4 = ({ userId }) => {
  const isTestAccount = userId?.startsWith('test');
  // 단계별 최소 머무름 시간 (ms) — test 계정은 0으로 스킵
  const MIN_DWELL_MS = isTestAccount ? {} : { intro: 4000, core: 6000, scenario: 4000, task: 4000 };
  const { missionId, gradeGroup: gradeBand } = useParams();
  const navigate = useNavigate();

  const mission = MISSIONS_V4[missionId];
  ...
  const validateCurrentStep = () => {
    ...
    if (step.uiMode === 'followup_question_carousel') {
      ...
      if (filteredBubbles.length === 0) return true;
      const allFilled = filteredBubbles.every(b => (answer?.[b.id] || '').trim());
      if (!allFilled) {
        setUiState(prev => ({ ...prev, validationError: '의심 말풍선마다 확인 질문을 작성해주세요.' }));
        return false;
      }
    }
    if (step.uiMode === 'per_case_judge' || step.uiMode === 'card_drop_board') {
      const pcjCases = deriveCases(step, gradeSpec, answers);
      const allDone = pcjCases.every(c => {
  ...
  const handlePrev = () => {
    ...
    if (stage === 'task') {
      ...
      if (stepIndex > 0) {
        ...
        setStage(gradeSpec.scenario?.image ? 'scenario' : 'core');
      }
      return;
    }
    if (stage === 'submit') {
...
export default MissionRunnerV4;
