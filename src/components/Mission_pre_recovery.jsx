...
const MISSIONS = {
    ...
    'E-2': {
        ...
        storySteps: {
            lower: [
                ...
                { text: "방금 AI 친구가 '호랑이가 세상에서 두 번째로 큰 동물'이라고 우겼대! 그게 정말일까?", image: '/robot_2d_base.png' },
            ...
            middle: [
                ...
                { text: "사람들은 이걸 '환각' 현상이라고 해. 확신에 차서 말하니까 속기 쉽지.", image: '/robot_2d_base.png' },
    ...
    'E-4': {
        ...
        prompts: {
            ...
            middle: ['만약 AI 얼굴 카메라가 어른만 빨리 인식해 통과시키고 어린이 얼굴은 하루 종일 인식 못 한다면, 이 AI를 만든 연구원들은 처음에 무슨 큰 실수를 저질렀던 걸까요?'],
    ...
    'M-3': {
        ...
        competency: 'AI 사용 판단 및 윤리',
        why: {
            lower: 'AI는 마법 지니처럼 뭐든 들어주지만, 말하는 대로만 해요! "케이크 만들어줘"라고 하면 아무 케이크나 뚝딱 만들지만, "딸기 장식 3층 초콜릿 케이크"라고 하면 내가 원하는 걸 정확히 만들 수 있어요.',
        ...
        storySteps: {
            ...
            upper: [
                { text: '안녕! 나는 프롬프트 엔지니어 알리야. 오늘은 AI를 정밀하게 조종하는 법을 배울 거야.', image: '/robot_2d_base.png' },
                { text: '"모호한 명령 = 쓰레기 결과"라는 법칙이 있어. 쓰레기가 들어가면 쓰레기가 나온다(GIGO)!', image: '/robot_2d_base.png' },
                { text: '하지만 [역할+과제+제약+형식]의 4요소 공식을 사용하면 AI가 마술처럼 정확한 답을 내놓아.', image: '/robot_2d_base.png' },
                { text: '이 공식을 "프롬프트 엔지니어링"이라고 해. 내 의도를 AI에게 정밀하게 전달하는 기술이지.', image: '/robot_2d_base.png' },
                { text: '나와 대화하며 4요소 공식을 연습해보자! 공식을 마스터하면 진짜 AI 조종사가 될 수 있어!', image: '/robot_2d_base.png' }
            ]
        },
        prompts: {
            lower: ['🏆 짧은 주문과 자세한 주문, 두 가지를 직접 만들어보자! 어떤 쪽이 AI에게 더 잘 전달될까요?'],
            middle: ['🎨 그림 주문서를 완성해 보아요! 조건을 고르고 나만의 완벽한 그림 주문 문장을 만들어 볼까요?'],
            upper: ['이 AI는 프롬프트 엔지니어링 코치예요! [역할+과제+제약+형식]의 4요소 공식으로 나만의 프롬프트를 완성해 보세요!']
        },
        type: { lower: 'stacked-inputs', middle: 'stacked-inputs', upper: 'chat' },
        isChatMode: { lower: false, middle: false, upper: true },
        chatInitiator: { lower: 'user', middle: 'user', upper: 'ai' },
        aiPromptText: `안녕하세요! 저는 프롬프트 엔지니어링 코치 알리예요. 오늘 제가 가르쳐 드릴 공식은 [역할 + 과제 + 제약 + 형식]이에요. 예를 들어: "초등학교 선생님처럼(역할), 기후변화를 설명해줘(과제), 10문장 이내로(제약), Q&A 형식으로(형식)" 이렇게요! 자, 이 공식을 연습하기 위해 아무 주제나 하나 골라서 프롬프트를 만들어 볼까요? 직접 써보면 제가 피드백해 드릴게요! ✍️`,
        persona: (missionTitle) => `당신은 학생들에게 프롬프트 엔지니어링을 가르치는 '친절한 AI 코치'입니다. 학생이 작성한 프롬프트를 분석하여 [역할+과제+제약+형식] 4요소 중 빠진 것이 있으면 짚어주고, 더 정밀한 프롬프트를 작성하도록 구체적인 피드백을 주세요. 학생이 4요소를 모두 포함한 훌륭한 프롬프트를 작성하면 "완벽한 프롬프트예요! 이제 진짜 프롬프트 엔지니어가 됐군요! 🎉"라고 칭찬하며 마무리하세요. 3~5번의 대화 안에 자연스럽게 완성하세요.`,
        stackedInputs: {
            lower: [
                { id: 'bad_prompt', type: 'text', label: '1. ❌ 짧고 성의없는 주문 (나쁜 예시)', placeholder: '힌트 없이 아주 짧게, 예: "케이크 만들어줘"' },
                { id: 'good_prompt', type: 'textarea', label: '2. ✅ 자세하고 구체적인 마법 주문서 (좋은 예시)', placeholder: '색상, 모양, 재료, 크기 등 자세한 단어를 가득 넣어 한 문장으로 써보세요!' }
            ],
            middle: [
                {
                    id: 'prompt_conditions',
                    type: 'checklist',
                    label: '🃏 그림에 넣고 싶은 조건 카드를 골라보세요! (여러 개 선택 가능)',
                    list: [
                        '🌈 색깔: 따뜻한 노란색과 오렌지색',
                        '🌙 배경: 별이 빛나는 밤하늘',
                        '🌊 배경: 넓고 푸른 바다',
                ...
                { id: 'final_prompt', type: 'textarea', label: '✍️ 고른 조건들을 모아 완성된 그림 주문서를 한 문장으로 써보세요!', placeholder: '예: 따뜻한 노란색으로, 별이 빛나는 밤하늘 아래 귀여운 고양이가 춤을 추는 동화책 그림을 그려줘!' }
            ]
        }
    },
    'M-4': {
...
export default function Mission({ userId, schoolId = 'gyeongdong', gradeGroup = 'lower', setFragments, onReward }) {
    ...
    const isCurrentChatMode = currentType === 'chat' || (typeof mission?.isChatMode === 'object' ? mission.isChatMode[gradeGroup] : mission?.isChatMode);
    ...
    const generateAIImage = async (promptKey) => {
        const userPrompt = stackedAnswers[promptKey];
        if (!userPrompt || (typeof userPrompt === 'string' && userPrompt.trim() === '')) {
            alert('먼저 주문서를 작성해 주세요! ✍️');
            return;
        }
        setIsGeneratingImage(true);
        setGeneratedImageUrl('');
        try {
            const imagePrompt = `Children's colorful illustration, 2D flat vector art style, cute and vibrant, no text, no letters. The scene: ${userPrompt}`;
            ...
            console.error('[Imagen] 이미지 생성 실패:', err);
            alert('이미지 생성에 실패했어요. 잠시 후 다시 시도해 주세요.');
        } finally {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // 2. Save submission data (Structured JSON for analysis)
            let submissionData = {
                file_url: fileUrl,
                research_data: {
            ...
            if (isCurrentChatMode) {
                submissionData.chat_log = messages;
                submissionData.text = messages.map(m => `[${m.role === 'user' ? '학생' : 'AI'}] ${m.content}`).join('\n');
            } else if (currentType === 'rules') {
                submissionData = { ...submissionData, rule1, rule2, rule3 };
                submissionData.text = `1. ${rule1}\n2. ${rule2}\n3. ${rule3}`;
            } else if (currentStackedInputs?.length > 0) {
                submissionData = { ...submissionData, stackedAnswers };
                submissionData.text = currentStackedInputs.map(input => {
                    const val = stackedAnswers[input.id] || '';
                    return `[${input.label}] ${Array.isArray(val) ? val.join(', ') : val}`;
                }).join('\n');
            } else {
                                    ...
                                    {currentStackedInputs?.length > 0 ? (
                                        <>
                                                        ...
                                                        {inputDef.type === 'checklist' ? (
                                                            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                                                                {inputDef.list?.map((item, idx) => {
                                                                    const currentVal = stackedAnswers[inputDef.id] || [];
                                                                    const isSelected = Array.isArray(currentVal)
                                                                        ? currentVal.includes(item)
                                                                        : currentVal.split(',').includes(item);
                                                                    return (
                                                                        <label key={idx} style={{
                                                                            display: 'flex', alignItems: 'center', gap: '10px',
                                                                            padding: '12px 15px',
                                                                            background: isSelected ? '#e3f2fd' : 'white',
                                                                            border: `2px solid ${isSelected ? '#74b9ff' : '#dfe6e9'}`,
                                                                            ...
                                                                            <input
                                                                                ...
                                                                                checked={isSelected}
                                                                                onChange={(e) => {
                                                                                    const current = Array.isArray(stackedAnswers[inputDef.id]) ? stackedAnswers[inputDef.id] : (stackedAnswers[inputDef.id] || '').split(',').filter(x => x);
                                                                                    const next = e.target.checked
                                                                                        ? [...current, item]
                                                                                        : current.filter(x => x !== item);
                                                                                    handleStackedChange(inputDef.id, next);
                                                                                }}
                                                                                style={{ width: '20px', height: '20px' }}
                                                                            />
                                                                            <span style={{ fontSize: '1rem', fontWeight: isSelected ? 'bold' : 'normal', color: '#2d3436' }}>{item}</span>
                                                                        </label>
                                                                    );
                                                                })}
                                                            </div>
                                                            ...
                                                            <textarea
                                                                ...
                                                                style={{ width: '100%', padding: '15px', borderRadius: '12px', border: '2px solid #dfe6e9', fontSize: '1.05rem', fontFamily: "'Nunito', sans-serif", resize: 'vertical' }}
                                                                required
                                                            />
                                                        ) : (
                                                            <input
                                            ...
                                            {missionId === 'M-3' && (gradeGroup === 'lower' || gradeGroup === 'middle') && (
                                                <div style={{ marginTop: '20px', padding: '20px', background: 'linear-gradient(135deg, #f5f7ff 0%, #ede7f6 100%)', borderRadius: '16px', border: '2px dashed #9c27b0' }}>
                                                    <div style={{ fontWeight: 'bold', color: '#6a0dad', marginBottom: '10px', fontSize: '1.05rem' }}>
                                                        🖼️ 내 주문서로 AI 그림 그려보기!
                                                    </div>
                                                    <p style={{ color: '#555', fontSize: '0.88rem', marginBottom: '14px', lineHeight: 1.5 }}>
                                                        {gradeGroup === 'lower'
                                                            ? '✨ "마법 주문서"를 완성했다면, AI가 그 주문대로 그림을 그려줄 거예요!'
                                                            : '✨ 완성된 "그림 주문서"를 바탕으로 AI가 그림을 그려줄 거예요!'}
                                                    </p>
                                                    <button
                                                        type="button"
                                                        onClick={() => generateAIImage(gradeGroup === 'lower' ? 'good_prompt' : 'final_prompt')}
                                                        disabled={isGeneratingImage}
                                                        style={{
                                                            background: isGeneratingImage ? '#bbb' : 'linear-gradient(135deg, #9c27b0, #673ab7)',
                                                    ...
                                                    >
                                                        {isGeneratingImage ? <><span>⏳</span> AI가 그림 그리는 중...</> : <>🎨 AI로 그림 그려보기!</>}
                                                    </button>
                                                    {generatedImageUrl && (
                                                        <div style={{ marginTop: '18px', textAlign: 'center' }}>
                                                            <div style={{ fontWeight: 'bold', color: '#6a0dad', marginBottom: '10px' }}>✨ AI가 그린 그림이에요!</div>
                                                            <img
                                                                src={generatedImageUrl}
                                                                alt="AI가 생성한 그림"
                                                                style={{ width: '100%', maxWidth: '380px', borderRadius: '16px', boxShadow: '0 8px 24px rgba(0,0,0,0.15)', border: '3px solid #9c27b0' }}
                                                            />
                                                            <p style={{ color: '#888', fontSize: '0.83rem', marginTop: '10px' }}>💡 주문이 자세할수록 AI 그림도 더 정확해져요!</p>
                                                        </div>
                                                    )}
                                                </div>
                                            )}
                                        ...
                                        currentType !== 'rules' && (
                                            <>
                                                ...
                                                {visibleHints['main_hint'] && currentExample && (
                                                    <div style={{
                                                        background: '#fff9c4', border: '2px dashed #fbc02d', borderRadius: '10px',
                                                        padding: '12px 15px', marginBottom: '15px', color: '#f57f17', fontSize: '0.95rem',
                                                        display: 'flex', alignItems: 'flex-start', gap: '8px', lineHeight: 1.5,
                                                        ...
                                                        <span style={{ fontSize: '1.2rem', marginTop: '-2px' }}>💁</span>
                                                        <span><strong>힌트 도우미:</strong> <VocabHighlighter text={currentExample} onWordClick={(word, desc) => setVocabModal({ show: true, word, desc })} /></span>
                                                    </div>
                                                )}
                                                <textarea
                                                    rows={4}
