/**
 * LearnAILIT V3 · E-4 모두에게 똑같이 잘할까?
 ...
 *
 * [통합 방향]
 * - E-4-L: '배려 관찰자' 역할 (상대 문서 채택) — 도움 방법이 어려움 판단과 연동
 * - E-4-M: '공정성 점검단원' 역할 (상대 문서 채택) + 개선 제안 단계 추가 (상대 문서 핵심 강점)
 * - E-4-H: '공정성 자문위원' 역할 (상대 문서 채택) + 기존 V3의 6단계 구조 완전 유지
 *          + 최종 자문 의견서 산출물 명시
 * - 공통: 루브릭 인식→근거→판단→책임→제안 축 구성
 */

export const E4_V3_SCENARIO = {
  meta: {
    code: "E-4",
    title: "모두에게 똑같이 잘할까?",
    domain: "Engaging",
  ...
  grades: {
    ...
    // =====================================================================
    // E-4-L | 저학년 (1~2학년)
    // 역할: AI 배려 관찰자 (상대 문서 채택)
    // 시나리오: 학교 체험관에서 같은 AI를 쓰는 두 친구의 경험 차이를 관찰하고 배려 방법 연결
    // 산출물: 어려움을 겪는 사람 선택 + 이유 선택 + 배려 방법 선택
    // 핵심 분기: step3 어려움 판단 → step4 해당 어려움에 맞는 배려 방법만 활성화
    // =====================================================================
    lower: {
      cardCode: "E-4-L",
      performanceType: "TD",
      ...
      steps: [
        ...
        },
        {
          ...
          feedback: {
            onCorrect: null,
            onWrong: "이건 AI 작동 방식과 직접 관계없는 차이예요. 다시 살펴볼까요?",
            onMiss: null
          },
        ...
        },
        {
          ...
          // [분기] 선택한 어려움 이유에 따라 step4 배려 방법이 달라짐
          validation: { required: true }
        },
        {
          id: "step4",
          title: "STEP 4 · 배려 방법 고르기",
          // [분기] step3에서 선택한 어려움에 맞는 배려 방법만 활성화
          question: "어떻게 하면 두 사람 모두 더 잘 쓸 수 있을까요?",
          hint: "한 가지 방법만 말고 다른 방법도 있으면 좋을까요?",
          uiMode: "choice_cards",
          randomize: true,
          // [연동] step3 이유와 연결되는 배려 방법 우선 표시
          // step3에서 고른 이유에 따라 관련 배려 옵션을 상단 강조
          // restrict → 비관련 옵션(change_color, make_bigger)은 비활성화
          branch: {
            sourceStepId: "step3",
            filterBy: "id",
            mode: "restrict",
      ...
      submit: {
        ...
        artifact: {
          ...
          template: "{step3}이 더 어려웠어요. {step4}"
        }
      }
    },
    ...
    // =====================================================================
    // E-4-M | 중학년 (3~4학년)
    // 역할: 공정성 점검단원 (상대 문서 채택)
    // 시나리오: 학교 AI 서비스 도입 전 공정성 점검 + 개선 제안 작성
    // 산출물: 불리한 학생 판단 + 공정성 판단 + 원인 선택 + 개선 제안
    // 핵심 추가: step4 개선 제안 단계 (상대 문서 핵심 강점)
    // =====================================================================
    middle: {
      cardCode: "E-4-M",
      performanceType: "SJ",
      ...
      steps: [
        ...
        },
        {
          ...
          cases: [
            { id: "face_recognition",  title: "사례 1 · 학교 출입 얼굴 인식",  description: "학생 B는 여러 번 시도해야 문이 열려요." },
            { id: "voice_recognition", title: "사례 2 · 숙제 찾기 학습 앱",    description: "학생 B는 말이 다르게 인식되어 엉뚱한 화면이 나와요." },
            { id: "auto_reading",      title: "사례 3 · 글을 읽어 주는 앱",    description: "학생 B는 속도가 맞지 않아 중요한 내용을 놓쳐요." }
          ],
      ...
      submit: {
        ...
        artifact: {
          ...
          template: "공정성 점검 결과와 개선 제안을 정리했어요."
        }
      }
    },
    ...
    // =====================================================================
    // E-4-H | 고학년 (5~6학년)
    // 역할: AI 공정성 자문위원 (상대 문서 채택)
    // 시나리오: 놀이공원 AI 서비스 4개를 점검하고 최종 자문 의견서 제출
    // 산출물: 장면별 분석표 + 영향 및 원인 정리 + 개선 절차 제안 + 최종 자문 의견서
    // 기존 V3 6단계 구조 완전 유지 + 역할·시나리오·산출물 레이어 추가
    // =====================================================================
    upper: {
      cardCode: "E-4-H",
      performanceType: "SJ",
      ...
      submit: {
        ...
        artifact: {
          bindingKey: "e_4_h_quality_order",
          template: "4개 장면 점검 완료. 최종 자문 의견서를 제출했어요."
        }
      }
    }
  }
